...
- Pipeline 代码清理：移除 `FetchOnlineStage` 未使用的 `reason` 变量；移除 `CreateRecordStage` 和 `RouteSourceStage` 未使用的 `services` 注入参数

## [2.17.1] - 2026-07-22

### Fixed

- 修复 H5 端海报保存到相册：改用 `uni.request` + `responseType: 'arraybuffer'` 获取图片数据创建 Blob URL，替代跨域直链导致浏览器跳转图片页面的问题
...
- 修复隐私弹窗不显示导致保存按钮卡死：去掉 `open-type="agreePrivacyAuthorization"`，改为普通按钮直接调用 `agreePrivacy()`

## [2.15.1] - 2026-07-08

### Fixed

- 修复拒绝隐私授权导致保存海报按钮卡死：`rejectPrivacy()` 调用 `resolve({ event: 'disagree' })` 正确结束授权流程
